...
The [`./create-template` script](#create-template) creates a Proxmox template using the `qm`
...
```bash
sudo mkdir /var/lib/vz/template/qcow && cd /var/lib/vz/template/qcow
sudo curl -LO https://dl.rockylinux.org/pub/rocky/10/images/x86_64/Rocky-10-GenericCloud-Base.latest.x86_64.qcow2
```

Then the script can be run directly (using `sudo`) without any options.  
```bash
...
the VM must be converted to a template manually. This is so further 
configuration can be done before templating the VM.  
...
Accepted options are as follows (all require arguments):
...
Create a file called `terraform.tfvars` and set the variables in this file.  
...
Required variables can alternatively be set in an environment file. A typical 
convention for this is a `.env` file.  
```bash
touch .env
vi .env
```
...
This file must be `source`d before running `terraform plan` or `terraform apply`.  
```bash
source .env
terraform plan
```
