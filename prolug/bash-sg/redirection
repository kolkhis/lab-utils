...
printf "Redirection, File I/O, heredocs/herestrings.\n"
...
printf "A common use case is piping to sed.\n"
    ...
    printf "Let's do some bash.\n" | sed 's/bash/redirection/' 
    # output: Let's do some redirection
...
printf "Note that the '>' operator will truncate the file (overwrite). So, use this carefully.\n"
printf "If you want to *append* to a file, use '>>' instead.\n"

    printf "This will not overwrite the file.\n" >> file.txt

printf "This will preserve the contents of the file and add a new line.\n"
...
# Sidebar: Read entire file into a variable
printf "How do we fix that? Set the delimiter to a null string.\n"

    read -r -d '' CONTENTS < file.txt
    printf "%s\n" "$CONTENTS"

printf "This tells 'read' to keep reading until EOF.\n"
printf "Now CONTENTS is a long string that contains the contents of the file.\n"
...
printf "What if we want to make sure the contents are stored line-by-line as an array?\n"
# TODO(fix): Update these examples

    # declare an array variable
    declare -a CONTENTS=()

    # Populate the array with the lines of the file
    read -r -d '' -a CONTENTS < file.txt
...
printf "A more realistic example:\n"
...
printf "Everything after the <<< is treated as a file. Just like a heredoc, except only 1 line.\n"

printf "This is really useful for performing operations on variables.\n"

    declare testvar='old old old'
    perl -pe 's/old/new/g' <<< "$testvar"
    # Rather than:
    echo "$testvar" | perl -pe 's/old/new/g'

printf "Here, we save a call to 'echo' and pass in the variable's contents directly.\n"
